...
public class ImpresionesDAO {
	...
	public List<Guia> buscarGuiasxUsuario(List<GuiasPendientes> guiasP, Connection con) {
		...
		String query = "SELECT CONVERT(varchar,CONVERT(varchar,G.FECHA,3),3) AS FECHA, G.NOMREM AS NOMREM, G.NOMDES AS NOMDES, G.DIRREM1+' '+G.DIRREM2 AS DIRREM, G.DIRDES1+' '+G.DIRDES2 AS DIRDES, "
				+ "G.MNCPORI AS MNCPORI, G.PTOORI AS PTOORI, G.MNCPDES AS MNCPDES, G.PTODES AS PTODES, G.TELREM AS TELREM, G.TELDES AS TELDES, G.COBEX AS COBEX, G.OBSERVACIONES AS OBSERVACIONES, "
				+ "G.CODCOB AS CODCOB, SUM(GD.PIEZAS) AS PIEZAS, SUM(GD.PIEZAS * GD.PESO) AS PESO, G.SEGURO AS SEGURO, G.DECLARADO AS DECLARADO, G.NOGUIA AS NOGUIA, G.LLAVECLIENTE AS LLAVECLIENTE, G.RECOGEOFICINA AS RECOGEOFICINA, "
				+ "G.COD_VALORACOBRAR AS COD_VALORACOBRAR, G.SEABREPAQUETE AS SEABREPAQUETE, FC.LXCOBRAR AS LXCOBRAR, FC.LPREPAGADA AS LPREPAGADA, FC.LCREDITO AS LCREDITO, FC.LCONTADOFRECUENTE AS LCONTADOFRECUENTE "
				+ "FROM GUIAS G INNER JOIN GUIASDETALLE GD ON G.NOGUIA = GD.NOGUIA "
				+ "INNER JOIN FACCLIENTES FC ON G.CODCOB = FC.CODIGO WHERE G.NOGUIA = ? "
				+ "GROUP BY G.FECHA, G.NOMREM, G.NOMDES, G.DIRREM1+' '+G.DIRREM2, G.DIRDES1+' '+G.DIRDES2, G.MNCPORI, G.PTOORI, G.MNCPDES, G.PTODES, "
				+ "G.TELREM, G.TELDES, G.COBEX, G.OBSERVACIONES, G.CODCOB, G.SEGURO, G.DECLARADO, G.NOGUIA, G.LLAVECLIENTE, "
				+ "G.RECOGEOFICINA, G.COD_VALORACOBRAR, G.SEABREPAQUETE, FC.LXCOBRAR, FC.LPREPAGADA, FC.LCREDITO, FC.LCONTADOFRECUENTE ";
	...
	public List<GuiaHija> buscarGuiasHija(String noGuia, Connection con) {
		...
		String query = "SELECT HGUIAHIJA, HNOGUIA, P_FECHA, P_HORA, P_ESTATUS, HESTATUS FROM JGUIASHIJAS where HNOGUIA = ? ";
